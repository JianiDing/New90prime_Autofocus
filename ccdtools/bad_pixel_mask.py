...
def process_and_save_calibrated_image(calibrated_image: np.ndarray, bad_columns: List[int], amp_num: float, output_path: str = './', show_plots: bool = True):
    ...
    print(f"Saved masked image to: {output_path}/masked_image_ampnum_"+str(amp_num)+'new.fits')
    ...
    print(f"Saved bad pixel map to: {output_path}/bad_pixel_map_ampnum_"+str(amp_num)+'new.fits')
